...
public ResponseEntity<PaymentLinkListResponse> getAllPaymentLinks(
    ...
    @GetMapping("/{id}")
    public ResponseEntity<?> getPaymentLinkById(@PathVariable("id") String id) {

        try {

            if (id == null || !id.startsWith("plink_")) {
                throw new IllegalArgumentException(
                        "invalid input [strippedId] = [" +
                                (id != null ? id : "null") + "]");
            }

            PaymentLinkResponse response =
                    paymentLinkService.getPaymentLinkById(id);
...
public ResponseEntity<?> cancelPaymentLink(@PathVariable("id") String id) {
    ...
    @PostMapping("/{id}/notify/{medium}")
    public ResponseEntity<?> sendOrResendNotification(
            @PathVariable String id,
            @PathVariable String medium) {

        try {
            Boolean result = paymentLinkService.sendOrResendNotification(id, medium);

            Map<String, Object> response = new HashMap<>();
            response.put("success", result);
            response.put("message", medium.toUpperCase() + " notification sent successfully");
